...
func TestToValue_String(t *testing.T) {
	...
	v, err := resolve.ToValue[string](expression)
	assert.NoError(t, err)
...
func TestToValue_NotString(t *testing.T) {
	...
	_, err = resolve.ToValue[string](expression)
...
func TestToValue_Number(t *testing.T) {
	...
	v, err := resolve.ToValue[int64](expression)
	assert.NoError(t, err)
...
func TestToValue_Float(t *testing.T) {
	...
	v, err := resolve.ToValue[float64](expression)
	assert.NoError(t, err)
...
func TestToValue_Boolean(t *testing.T) {
	...
	v, err := resolve.ToValue[bool](expression)
	assert.NoError(t, err)
