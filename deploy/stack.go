...
type NewLocalWorkspaceArgs struct {
	Config       *config.ProjectConfig
	PulumiConfig *PulumiConfig
}

func newLocalWorkspace(ctx context.Context, args *NewLocalWorkspaceArgs) (auto.Workspace, error) {
	ws, err := auto.NewLocalWorkspace(ctx, args.PulumiConfig.WorkspaceOptions...)
	if err != nil {
		return nil, err
	}

	err = ws.SetConfig(ctx, args.PulumiConfig.StackName, "aws:region", auto.ConfigValue{
		Value: args.Config.Deploy.Region,
	})
	if err != nil {
		return nil, err
	}
...
func selectStack(ctx context.Context, args *SelectStackArgs) (*auto.Stack, error) {
	...
	ws, err := newLocalWorkspace(ctx, &NewLocalWorkspaceArgs{
		Config:       args.Config,
		PulumiConfig: args.PulumiConfig,
	})
	if err != nil {
		return nil, err
	}

	ws.SetProgram(runFunc)
