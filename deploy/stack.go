...
func selectStack(ctx context.Context, args *SelectStackArgs) (*auto.Stack, error) {
	...
	if err != nil {
		log("%s error creating workspace: %s", IconCross, err.Error())
		return nil, err
	}
	...
	if err != nil {
		log("%s error setting config aws:region on stack: %s", IconCross, err.Error())
		return nil, err
	}
...
func getStackOutputs(ctx context.Context, args *GetStackOutputsArgs) (*StackOutputs, error) {
	...
	if err != nil {
		log("%s error creating Pulumi workspace: %s", IconCross, gray(err.Error()))
		return nil, err
	}
	...
	if err != nil {
		log("%s error selecting stack: %s", IconCross, gray(err.Error()))
		return nil, err
	}

	outputs, err := stack.Outputs(ctx)
	if err != nil {
		log("%s error getting stack outputs: %s", IconCross, gray(err.Error()))
		return nil, err
	}
