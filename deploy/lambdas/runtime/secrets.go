...
func initSecrets(ctx context.Context, names []string, awsEndpoint string) (map[string]string, error) {
	...
	secretNames := lo.Map(names, func(s string, _ int) string {
		return SsmParameterName(os.Getenv("KEEL_PROJECT_NAME"), os.Getenv("KEEL_ENV"), s)
	})
