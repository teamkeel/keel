...
func createProgram(args *NewProgramArgs) pulumi.RunFunc {
	return func(ctx *pulumi.Context) error {
		...
		// update from config
		if deployCfg.Database != nil {
			db := deployCfg.Database
		...
		if deployCfg.Telemetry != nil && deployCfg.Telemetry.Collector != "" {
		...
		// OTEL config
		if deployCfg.Telemetry != nil && deployCfg.Telemetry.Collector != "" {
			functionEnvVars["OPENTELEMETRY_COLLECTOR_CONFIG_FILE"] = pulumi.String("/var/task/collector.yaml")
		}
		...
		// OTEL config
		if deployCfg.Telemetry != nil && deployCfg.Telemetry.Collector != "" {
			baseRuntimeEnvVars["OPENTELEMETRY_COLLECTOR_CONFIG_FILE"] = pulumi.String("/var/task/collector.yaml")
		}
