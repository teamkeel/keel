  ...
  return opentelemetry.context.with(activeContext, () => {
    ...
    return withSpan(request.method, async (span) => {
      try {
        ...
        const { permissionState } = request.meta || { permissionState: { status: "unknown" } };
        const permitted = permissionState !== null && permissionState.status === "granted" ? true: null;

        const db = getDatabase();
        const permissions = new Permissions();

        const result = await permissionsApiInstance.run({ permitted: permitted }, () => {
