...
func NewPackageJson(path string, packageManager PackageManager) (*PackageJson, error) {
	p := PackageJson{
		Path:           path,
	...
	if _, err := os.Stat(path); errors.Is(err, os.ErrNotExist) {
		cmd := exec.Command(string(packageManager), "init")
...
func (p *PackageJson) Install() error {
	npmInstall := exec.Command(string(p.PackageManager), "install")
