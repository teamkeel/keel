...
var deployBuildCommand = &cobra.Command{
	Use:   "build",
	Short: "Build the resources that are needed to deploy your app to AWS account",
	Args:  cobra.NoArgs,
	Run: func(cmd *cobra.Command, args []string) {
		defer panicHandler()
		...
		if err != nil {
			fmt.Println("error running build", err)
			os.Exit(1)
		}
	},
}
...
var deploySecretsGetCommand = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		param, err := deploy.GetSecret(context.Background(), &deploy.GetSecretArgs{
			ProjectRoot: validated.ProjectDir,
			Env:         validated.Env,
			Key:         args[0],
		})
