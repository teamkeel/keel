...
func UpdateFunctions(schema *proto.Schema, dir string, withQueryModule bool) tea.Cmd {
	return func() tea.Msg {
		files, err := node.Generate(context.TODO(), schema, node.WithDevelopmentServer(true), node.WithQueryModule(withQueryModule))
		if err != nil {
