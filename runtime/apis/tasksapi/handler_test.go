...
func TestUnassignTask_Success(t *testing.T) {
	...
	request := makeUnassignRequest(ctx, task.ID, accessToken)
...
func TestUnassignTask_TaskNotFound(t *testing.T) {
	...
	request := makeUnassignRequest(ctx, "non-existent-id", accessToken)
...
func makeUnassignRequest(ctx context.Context, taskID string, accessToken string) *http.Request {
	request := httptest.NewRequest(http.MethodPut, "http://mykeelapp.keel.so/topics/json/TestTask/tasks/"+taskID+"/unassign", nil)
	request.Header.Add("Content-Type", "application/json")
	request.Header.Add("Authorization", "Bearer "+accessToken)
	request = request.WithContext(ctx)
	return request
