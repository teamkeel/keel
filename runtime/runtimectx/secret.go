...
func GetCtxSecret(ctx context.Context, secret string) (string, error) {
	v := ctx.Value(SecretContextKey)
	if v == nil {
		return "", fmt.Errorf("context does not have a :%s key", dbKey)
	}
...
func GetCtxSecrets(ctx context.Context) map[string]string {
	return ctx.Value(SecretContextKey).(map[string]string)
}

func SetCtxSecrets(ctx context.Context, secrets map[string]string) context.Context {
