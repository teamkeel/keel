...
var rpcTestCases = []rpcTestCase{
	...
	{
		name: "rpc_json_invalid_nullable_input_isNull_true_with_value",
		keelSchema: `
		model Thing {
			fields {
				text Text?
	...
	`,
		Path:   "createThing",
		Body:   `{"text": { "value": "foo", "isNull": true}}`,
		Method: http.MethodPost,
		assertError: func(t *testing.T, data map[string]any, statusCode int) {
			assert.Equal(t, statusCode, http.StatusBadRequest)
			rtt.AssertValueAtPath(t, data, "code", "ERR_INVALID_INPUT")
			rtt.AssertValueAtPath(t, data, "message", "invalid value for 'text': nullable input cannot have a value if isNull is true")
		},
	},
	{
		name: "rpc_json_invalid_nullable_input_not_nullable",
		keelSchema: `
		model Thing {
			fields {
				text Text?
	...
	`,
		...
		assertError: func(t *testing.T, data map[string]any, statusCode int) {
			assert.Equal(t, statusCode, http.StatusBadRequest)
			assert.Equal(t, "ERR_INVALID_INPUT", data["code"])
			rtt.AssertValueAtPath(t, data, "data.errors[0].field", "text")
			rtt.AssertValueAtPath(t, data, "data.errors[0].error", "Invalid type. Expected: object, given: string")
		},
	},
	{
		name: "rpc_json_invalid_nullable_input_empty_object",
		keelSchema: `
		model Thing {
			fields {
				text Text?
	...
	`,
		...
		assertError: func(t *testing.T, data map[string]any, statusCode int) {
			assert.Equal(t, statusCode, http.StatusBadRequest)
			rtt.AssertValueAtPath(t, data, "code", "ERR_INVALID_INPUT")
			rtt.AssertValueAtPath(t, data, "message", "invalid value for 'text': nullable input must have a value or isNull set")
		},
	},
	{
		name: "rpc_json_invalid_nullable_input_missing_value",
		keelSchema: `
		model Thing {
			fields {
				text Text?
	...
	`,
		Path:   "createThing",
		Body:   `{"text": { "isNull": false}}`,
		Method: http.MethodPost,
		assertError: func(t *testing.T, data map[string]any, statusCode int) {
			assert.Equal(t, statusCode, http.StatusBadRequest)
			rtt.AssertValueAtPath(t, data, "code", "ERR_INVALID_INPUT")
			rtt.AssertValueAtPath(t, data, "message", "invalid value for 'text': nullable input must have a value if isNull is false")
		},
	},
}
