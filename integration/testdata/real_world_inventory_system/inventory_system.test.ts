...
beforeAll(async () => {
    ...
    samsungIdentity = await models.identity.create({ email: "samsung@myshop.co.uk"});
    ...
    await models.user.create({ identityId: samsungIdentity.id, brandId: samsung.id });
...
test('create product with duplicated product code - invalid inputs response', async () => {
    ...
    await expect( 
        ...
        message: "the values for the unique composite fields (brandId, productCode) must be unique"
    });
...
test('create product without brand permissions - invalid inputs response', async () => {
    const samsung = await actions.getBrand({ code: "SMSG"});
    expect(samsung).not.toBeNull();
...
test('list suppliers with a certain product - suppliers returned', async () => {
    const suppliers = await actions.suppliersWithProduct({ where: {
        catalog: {
            product: {
                barcode: {
                    equals: "346123498729"
...
test('deactive product without brand access - permission error', async () => {
   await expect(actions.withIdentity(appleIdentity).deactivateProduct({
        where: {
            brandCode: "SMSG",
            productCode: "00002"
        }
...
test('deactive product - product deactived', async () => {
    const product = await actions.withIdentity(samsungIdentity).deactivateProduct({
        where: {
            brandCode: "SMSG",
            productCode: "00002"
        }
...
test('search catalog - catalog items returned', async () => {
    const tv = await actions.search({ where: {
            product: {
                name: {
                    contains: "TV"
                },
    ...
    expect(tv.pageInfo.count).toEqual(0);

    const appleItems = await actions.search({ 
        where: {
            product: {
                name: {
                    contains: "Apple"
...
test('delete product without brand access - permission error', async () => {
    await  expect(actions.withIdentity(samsungIdentity).deleteProduct({
        brandCode: "APL",
        productCode: "IPH15"
    })).toHaveAuthorizationError();
