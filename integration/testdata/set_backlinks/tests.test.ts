...
test("update - @set with backlinks", async () => {
    const orgKeel = await models.organisation.create({ name: "Keel", isActive: true  });
    const identityKeelson = await models.identity.create({ email: "keelson@keel.so" });
    const userKeelson = await models.user.create({ name: "Keelson", identityId: identityKeelson.id, organisationId: orgKeel.id });

    const record = await actions.withIdentity(identityKeelson).createRecord({ 
        name: "Tax Records"
    });

    const orgWeave = await models.organisation.create({ name: "Weave", isActive: false  });
    const identityWeaveton = await models.identity.create({ email: "weaveton@keel.so" });
    const userWeaveton = await models.user.create({ name: "Weaveton", identityId: identityWeaveton.id, organisationId: orgWeave.id });

    const updatedRecord = await actions.withIdentity(identityWeaveton).updateRecordOwner({ 
        where: { id: record.id }
    });
