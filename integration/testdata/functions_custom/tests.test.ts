...
test("updatePhoto", async () => {
  ...
  const result = await actions.updatePhoto({
    id: keelio.id,
    photo: InlineFile.fromDataURL(dataUrl),
  });
...
test("readPhoto", async () => {
  ...
  const read = await actions.readFile({ id: keelio.id });
  expect(read.contents).toBeTruthy();
  expect(read.photo.size).toEqual(2024);
});

test("complex input and response messages", async () => {
  const response = await actions.readComplex({
    msg: {
      texts: ["A", "B"],
    },
    msgArray: [
