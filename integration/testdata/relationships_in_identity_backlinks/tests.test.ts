...
test("relationships and backlinks - in operator", async () => {
    ...
    const johnIsFollowing = await actions.withIdentity(johnIdentity).accountsFollowed();
    ...
    const arthurIsFollowing = await actions.withIdentity(arthurIdentity).accountsFollowed();
    expect(arthurIsFollowing.results).toHaveLength(1);
    expect(arthurIsFollowing.results[0].username).toEqual("mary77");


    const maryIsFollowing = await actions.withIdentity(maryIdentity).accountsFollowed();
    expect(maryIsFollowing.results).toHaveLength(0);


    const followingJohn = await actions.withIdentity(johnIdentity).accountsFollowingMe();
    expect(followingJohn.results).toHaveLength(0);


    const followingArthur = await actions.withIdentity(arthurIdentity).accountsFollowingMe();
    expect(followingArthur.results).toHaveLength(1);
    expect(followingArthur.results[0].username).toEqual("johndoe");



    const followingMary = await actions.withIdentity(maryIdentity).accountsFollowingMe();
...
test("relationships and backlinks - not in operator", async () => {
  ...
  const johnIsNotFollowing = await actions.withIdentity(johnIdentity).accountsNotFollowed();
  expect(johnIsNotFollowing.results).toHaveLength(0);


  const arthurIsNotFollowing = await actions.withIdentity(arthurIdentity).accountsNotFollowed();
  expect(arthurIsNotFollowing.results).toHaveLength(1);
  expect(arthurIsNotFollowing.results[0].username).toEqual("johndoe");



  const maryIsNotFollowing = await actions.withIdentity(maryIdentity).accountsNotFollowed();
  expect(maryIsNotFollowing.results).toHaveLength(2);


  const notFollowingJohn = await actions.withIdentity(johnIdentity).accountsNotFollowingMe();
  ...
  const notFollowingArthur = await actions.withIdentity(arthurIdentity).accountsNotFollowingMe();
  expect(notFollowingArthur.results).toHaveLength(1);
  expect(notFollowingArthur.results[0].username).toEqual("mary77");


  const notFollowingMary = await actions.withIdentity(maryIdentity).accountsNotFollowingMe();
