...
export default ModelApiTests(async (ctx, inputs) => {
    ...
    const file = new InlineFile({ filename: "file.csv", contentType: 'text/csv'});
    ...
    const whereFilesQB = await models.myFile.where({ id : createdFile.id }).findMany();
    const b4 = await whereFilesQB[0]?.file?.read();
    if(b4?.toString("utf-8") != "hello world") {
        throw new Error("reading a file retrieived using models.where.findMany (the query builder) failed");
    }
    ...
    const updatedFile = await models.myFile.update({ id: createdFile.id }, { file: stored });
    const b6 = await updatedFile.file?.read();
    if(b6?.toString("utf-8") != "goodbye world") {
        throw new Error("reading a stored file updated using models.updated failed");
    }

    const updatedFile2 = await models.myFile.update({ id: createdFile.id }, { file: file! });
    const b10 = await updatedFile.file?.read();
    if(b10?.toString("utf-8") != "goodbye again world") {
        throw new Error("reading a inline file updated using models.updated failed");
    }
    ...
    const b8 = await findFile?.file?.read();
