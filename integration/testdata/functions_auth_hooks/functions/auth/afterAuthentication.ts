...
export default AfterAuthentication(async (ctx) => {
    ...
    if (ctx.isAuthenticated) {
        ...
        const account = await models.account.where({ identityId: ctx.identity!.id }).findOne();
        if (account) {
            
            const newCount =  account.loginCount + 1;
           const updated = await models.account.update({ id: account.id}, { loginCount : newCount });
        }
    } else {
