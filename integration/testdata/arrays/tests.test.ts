...
test("array fields - list action implicit querying - text", async () => {
  ...
  expect(things8.results).toHaveLength(2);
  expect(things8.results[0].id).toEqual(t5.id);
  expect(things8.results[1].id).toEqual(t7.id);
  ...
  expect(things11.results).toHaveLength(4);
  ...
  expect(things11.results[3].id).toEqual(t6.id);
});
